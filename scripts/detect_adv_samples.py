...
def main(args):
    ...
    print('20000-10000, Detector ROC-AUC score: %0.4f' % auc_score)

    _, _, auc_score = compute_roc(
        probs_neg=probs[:2 * n_samples],
        probs_pos=probs[2 * n_samples: 2 * n_samples + 1000]
    )
    print('20000-1000, Detector ROC-AUC score: %0.4f' % auc_score)

    # only use 1000 samples
    # neg1 = probs[0: n_samples]
    # neg2 = probs[n_samples: 2 * n_samples]
    neg1 = probs[0: 1000]
    neg_append = np.append(neg1, probs[n_samples: n_samples+1000])
    pos = probs[2 * n_samples: 2 * n_samples + 1000]

    _, _, auc_score = compute_roc(
        probs_neg=neg_append,
        probs_pos=pos
    )
    print('2000-1000, Detector ROC-AUC score: %0.4f' % auc_score)
